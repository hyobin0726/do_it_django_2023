...
class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.user_ahyobin = User.objects.create_user(username='ahyobin',password = 'somepassword')
        self.user_bhyobin = User.objects.create_user(username='bhyobin', password='somepassword')

        self.user_bhyobin.is_staff = True
        self.user_bhyobin.save()

        self.category_cafe = Category.objects.create(name='cafe', slug='cafe')
        self.category_cooking = Category.objects.create(name='cooking', slug='cooking')
        ...
        self.post_002 = Post.objects.create(
            ...
            category=self.category_cooking,
            author=self.user_bhyobin
        )
    ...
    def category_card_test(self, soup):
        ...
        self.assertIn(
                f'{self.category_cafe.name} ({self.category_cafe.post_set.count()})',
                categories_card.text
            )
        self.assertIn(
                f'{self.category_cooking.name} ({self.category_cooking.post_set.count()})',
                categories_card.text
            )
    ...
    def test_post_detail(self):
        ...
        self.assertIn(self.category_cafe.name, post_area.text)

        # 1.5  첫 번째 post의 작성자(author)가 post-area에 있다.
        # 아직 작성 불가
        self.assertIn(self.user_ahyobin.username.upper(), post_area.text)
    ...
    def test_category_page(self):
        response = self.client.get(self.category_cafe.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        ...
        self.assertIn(self.category_cafe.name, soup.h1.text)

        main_area = soup.find('div', id='main-area')
        self.assertIn(self.category_cafe.name, main_area.text)
    ...
    def test_create_post(self):
        ...
        self.client.login(username='ahyobin', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)
        ...
        self.assertEqual(last_post.author.username, 'bhyobin')
        ...
        def test_update_post(self):
            ...
            self.client.login(
                username=self.user_ahyobin.username,
            ...
            response = self.client.post(
                update_post_url,
                {
                    ...
                    'category': self.category_cafe.pk,
    ...
    def test_comment_form(self):
        ...
        self.assertIn('bhyobin', new_comment_div.text)
        self.assertIn('효빈의 댓글입니다.', new_comment_div.text)
        ...
        def test_comment_update(self):
            ...
            self.assertIn('효빈이의 댓글을 수정합니다.', comment_001_div.text)
            self.assertIn('Updated: ', comment_001_div.text)

        def test_delete_comment(self):
                comment_by_ahyobin = Comment.objects.create(
                    post=self.post_001,
                    author=self.user_ahyobin,
                    content='효빈이의 댓글입니다.'
                )
                ...
                self.assertIn(self.post_001.title, soup.title.text)
                comment_area = soup.find('div', id='comment-area')
                self.assertNotIn('효빈이의 댓글입니다.', comment_area.text)
